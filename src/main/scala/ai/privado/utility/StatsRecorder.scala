...
class StatsRecorder(
  ...
  // This flag helps to print the stats only for the passes being used in application.
  ...
  /** Stage `invoked` and `done` status's can be logged to both console logger file. By default it will always log to
    * logger file, printing the logs to console can be controlled using flag {@link StatsRecorder#printToConsole}
    */
  ...
  /** This will facilitate initiating the stats recording with configured regular frequency inside given csv file path.
    *
    ...
    */
  def startRecordingWithGivenFrequency(timeMetricRecordConfig: Option[TimeMetricRecordConfig] = None): Unit = {
  ...
  /** This will help change the flag once it has been set initially using {@link StatsRecorder# initialize()} method.
    *
    * @param flag
  ...
  /** API to signal start of the stage. This same method is used to initiate parent stage as well as sub stage. If this
    ...
    * @param stageFullName
    *   \- class full name to use as stage when {@link StatsRecorder#fullName} flag is set.
  ...
  /** This will be mostly useful to log info logs to console.
    * @param message
    */
