...
object JSONExporter {
  ...
  def fileExport(
    ...
    try {
      output.addOne(Constants.coreVersion   -> Environment.privadoVersionCore.getOrElse("Not Detected").asJson)
      output.addOne(Constants.cliVersion    -> Environment.privadoVersionCli.getOrElse("Not Detected").asJson)
