...
class SourceExporter(cpg: Cpg) {
  ...
  def getProcessing = {
    val processingMap = HashMap[String, mutable.Set[CfgNode]]()
    ...
    processingMap.map(entrySet =>
      mutable.LinkedHashMap[String, Json](
        ...
        Constants.occurrences -> ExporterUtility.convertPathElement(entrySet._2.toList.distinctBy(_.code).distinctBy(_.lineNumber).distinctBy(_.location.filename)).asJson
      )
    )
  }
