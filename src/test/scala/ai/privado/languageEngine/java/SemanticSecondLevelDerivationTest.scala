...
import io.joern.dataflowengineoss.semanticsloader.{FlowMapping, FlowSemantic, Semantics}
  ...
  "Semantic generated for 2nd Level derivation" should {
    ...
    "have non-personal semantics for 1st Level class" in {
      semantics.elements.contains(
        FlowSemantic("BaseClass.getId:java.lang.String()", List(FlowMapping(0, 0)))
      ) shouldBe true
    }
    ...
    "have non-personal semantics for 2nd Level class by member" in {
      semantics.elements.contains(
        FlowSemantic("Customer.getType:java.lang.String()", List(FlowMapping(0, 0)))
      ) shouldBe true
    }
