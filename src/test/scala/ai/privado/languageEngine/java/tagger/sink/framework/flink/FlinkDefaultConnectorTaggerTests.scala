...
class FlinkDefaultConnectorTaggerTests extends JavaFrontendTestSuite {
  ...
  "Flink default connector present in the same method as the flink sink" should {
    val cpg = code("""
        ...
        |        FlinkKafkaProducer<String> kafkaProducer = new FlinkKafkaProducer<String>("kafka-topic", new SimpleStringSchema(), kafkaProps);
        |
